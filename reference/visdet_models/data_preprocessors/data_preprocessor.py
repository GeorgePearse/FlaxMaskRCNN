...
from visdet.models.utils.misc import samplelist_boxtype2tensor
...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def __init__(
        self,
        mean: Sequence[Number] | None = None,
        std: Sequence[Number] | None = None,
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        logger.debug(f"[DetDataPreprocessor] Processing batch with {inputs.shape[0]} images")
        logger.debug(f"[DetDataPreprocessor] Input shape: {inputs.shape}, dtype: {inputs.dtype}")
        ...
        if data_samples is not None:
            ...
            for i, (data_sample, pad_shape) in enumerate(zip(data_samples, batch_pad_shape, strict=False)):
                data_sample.set_metainfo({"batch_input_shape": batch_input_shape, "pad_shape": pad_shape})

                # Debug log ground truth info
                if hasattr(data_sample, "gt_instances") and training:
                    gt_instances = data_sample.gt_instances
                    num_gt = len(gt_instances.bboxes) if hasattr(gt_instances, "bboxes") else 0
                    logger.debug(f"[DetDataPreprocessor] Sample {i}: {num_gt} ground truth boxes")
                    ...
                    if hasattr(gt_instances, "bboxes") and len(gt_instances.bboxes) > 0:
                        ...
                        if isinstance(bboxes, torch.Tensor):
                            if torch.isnan(bboxes).any():
                                logger.error(f"[DetDataPreprocessor] NaN detected in GT bboxes for sample {i}!")
                            if torch.isinf(bboxes).any():
                                logger.error(f"[DetDataPreprocessor] Inf detected in GT bboxes for sample {i}!")
        ...
        if training and self.batch_augments is not None:
            logger.debug(f"[DetDataPreprocessor] Applying {len(self.batch_augments)} batch augmentations")
    ...
    def _get_pad_shape(self, data: dict) -> list[tuple]:
        ...
        # Process data with `default_collate`.
            ...
            assert _batch_inputs.dim() == 4, (
                f"The input of `ImgDataPreprocessor` should be a NCHW tensor or a list of tensor, but got a tensor with shape: {_batch_inputs.shape}"
            )
...
class BatchSyncRandomResize(nn.Module):
    ...
    def forward(self, inputs: Tensor, data_samples: list[DetDataSample]) -> tuple[Tensor, list[DetDataSample]]:
...
class BatchFixedSizePad(nn.Module):
    ...
    def forward(self, inputs: Tensor, data_samples: list[dict] | None = None) -> tuple[Tensor, list[dict] | None]:
...
class MultiBranchDataPreprocessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        for key in data.keys():
            for branch in data[key].keys():
                data[key][branch] = list(filter(lambda x: x is not None, data[key][branch]))
    ...
    def to(self, device: int | torch.device | None, *args, **kwargs) -> nn.Module:
...
class BoxInstDataPreprocessor(DetDataPreprocessor):
    ...
    def get_images_color_similarity(self, inputs: Tensor, image_masks: Tensor) -> Tensor:
        """Compute the image color similarity in LAB color space."""
        assert inputs.dim() == 4
        assert inputs.size(0) == 1

        unfolded_images = unfold_wo_center(inputs, kernel_size=self.pairwise_size, dilation=self.pairwise_dilation)
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if training:
            ...
            for i in range(inputs.shape[0]):
                ...
                pixels_removed = int(self.bottom_pixels_removed * float(img_h) / float(b_img_h))
                if pixels_removed > 0:
            ...
            for im_i, data_sample in enumerate(data_samples):
                ...
                images_lab = skimage.color.rgb2lab(downsampled_imgs[im_i].byte().permute(1, 2, 0).cpu().numpy())
                images_lab = torch.as_tensor(images_lab, device=ori_imgs.device, dtype=torch.float32)
                images_lab = images_lab.permute(2, 0, 1)[None]
                images_color_similarity = self.get_images_color_similarity(images_lab, img_masks[im_i])
                pairwise_mask = (images_color_similarity >= self.pairwise_color_thresh).float()

                per_im_bboxes = data_sample.gt_instances.bboxes
                if per_im_bboxes.shape[0] > 0:
                    per_im_masks = []
                    for per_box in per_im_bboxes:
                        mask_full = torch.zeros((b_img_h, b_img_w), device=self.device).float()
                        mask_full[
                    ...
                    pairwise_masks = torch.cat([pairwise_mask for _ in range(per_im_bboxes.shape[0])], dim=0)
                else:
                    per_im_masks = torch.zeros((0, b_img_h, b_img_w))
                    pairwise_masks = torch.zeros((0, self.pairwise_size**2 - 1, b_img_h, b_img_w))

                # TODO: Support BitmapMasks with tensor?
                data_sample.gt_instances.masks = BitmapMasks(per_im_masks.cpu().numpy(), b_img_h, b_img_w)
